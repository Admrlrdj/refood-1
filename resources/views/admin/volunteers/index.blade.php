...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .stat-top-name { font-size:0.95rem; font-weight:800; margin-top:4px; }
        .stat-top-sub  { font-size:0.72rem; opacity:.85; }
    ...
    <div class="content">
        ...
        <div class="stats-grid">
            <div class="stat-card orange">
                <div class="stat-label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8.56 2.75c4.37 6.03 6.02 9.42 8.03 17.72"/></svg>Total Volunteers</div>
                <div class="stat-number">{{ $stats['total'] }}</div>
            </div>
            <div class="stat-card green">
                <div class="stat-label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>Active Volunteers</div>
                <div class="stat-number">{{ $stats['active'] }}</div>
            </div>
            <div class="stat-card blue">
                <div class="stat-label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="m16 8 5 3v5h-5V8z"/></svg>Total Deliveries</div>
                <div class="stat-number">{{ $stats['total_deliveries'] }}</div>
            </div>
            <div class="stat-card purple">
                <div class="stat-label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>Top Volunteer</div>
                @if($stats['top_volunteer'])
                <div class="stat-top-name">{{ Str::limit($stats['top_volunteer']->name, 12) }}</div>
                <div class="stat-top-sub">{{ $stats['top_volunteer']->deliveries_count }} deliveries</div>
                @else
        ...
        <div class="table-card">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $vol->deliveries_count ?? 0 }}</td>
                        <td class="td-muted">
                            {{ $vol->deliveries->last()?->created_at?->format('d M Y') ?? '-' }}
                        </td>
                        <td>
                            @if(($vol->deliveries_count ?? 0) > 0)
                        ...
                        <td>
                            <a href="{{ route('admin.volunteers.show', $vol) }}" class="btn-view">
                    ...
                    @empty
                    <tr><td colspan="5" style="text-align:center;padding:30px;color:var(--muted);">Belum ada data volunteer</td></tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="pagination-wrap">
                ...
                @else
                    <a href="{{ $volunteers->previousPageUrl() }}" class="page-btn arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><polyline points="15 18 9 12 15 6"/></svg></a>
                @endif
                ...
                @if($volunteers->hasMorePages())
                    <a href="{{ $volunteers->nextPageUrl() }}" class="page-btn arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><polyline points="9 18 15 12 9 6"/></svg></a>
                @else
