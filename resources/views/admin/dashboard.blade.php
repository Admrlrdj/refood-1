...
<head>
    ...
    <title>Dashboard - RE-FOOD Admin</title>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* STAT CARDS — 4 kolom sesuai gambar */
        .stats-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:14px; }
        .stat-card { border-radius:12px; padding:16px 18px; color:#fff; position:relative; overflow:hidden; display:flex; align-items:center; justify-content:space-between; }
        .stat-card::after { content:''; position:absolute; right:-15px; top:-15px; width:75px; height:75px; border-radius:50%; background:rgba(255,255,255,0.12); }
        .stat-card.green  { background:linear-gradient(135deg,#43a047,#2e7d32); }
        .stat-card.orange { background:linear-gradient(135deg,#ffb300,#e65100); }
        .stat-card.purple { background:linear-gradient(135deg,#ab47bc,#6a1b9a); }
        .stat-card.teal   { background:linear-gradient(135deg,#26c6da,#00838f); }
        .stat-left { position:relative; z-index:1; }
        .stat-label { display:flex; align-items:center; gap:6px; font-size:0.72rem; font-weight:600; opacity:.9; margin-bottom:6px; }
        ...
        .stat-badge { background:rgba(255,255,255,0.25); border-radius:8px; width:40px; height:40px; display:flex; align-items:center; justify-content:center; }
        ...
        .btn-view-all { font-size:0.8rem; font-weight:600; color:#2e7d32; text-decoration:none; }
    ...
    <div class="content">
        ...
        <div class="stats-grid">
            <div class="stat-card green">
                ...
                <div class="stat-icon-wrap"><div class="stat-badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg></div></div>
            </div>
            <div class="stat-card orange">
                <div class="stat-left">
                    <div class="stat-label">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8.56 2.75c4.37 6.03 6.02 9.42 8.03 17.72m2.54-15.38c-3.72 4.35-8.94 5.66-16.88 5.85"/></svg>
                        Total Volunteers
                    </div>
                    <div class="stat-number">{{ $totalVolunteers }}</div>
                </div>
                <div class="stat-icon-wrap"><div class="stat-badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8.56 2.75c4.37 6.03 6.02 9.42 8.03 17.72m2.54-15.38c-3.72 4.35-8.94 5.66-16.88 5.85m19.5 1.9c-3.5-.93-6.63-.82-8.94 0-2.58.92-5.01 2.86-7.44 6.32"/></svg></div></div>
            </div>
            <div class="stat-card purple">
                ...
                <div class="stat-icon-wrap"><div class="stat-badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div></div>
            </div>
            <div class="stat-card teal">
                <div class="stat-left">
                    ...
                    <div class="stat-number">{{ $totalReceivers }}</div>
                </div>
                <div class="stat-icon-wrap"><div class="stat-badge"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div></div>
            </div>
        </div>

        <!-- RECENT DELIVERIES TABLE — kolom sesuai gambar: Food Item, Volunteer, Receiver, Status, Date -->
        <div class="table-card">
            <div class="table-card-header">
                <h3>Recent Deliveries</h3>
                <a href="{{ route('admin.deliveries.index') }}" class="btn-view-all">View All →</a>
            </div>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr onclick="window.location='{{ route('admin.deliveries.show', $d) }}'">
                        <td class="td-food">{{ $d->food->name ?? '-' }}</td>
                        <td>{{ $d->volunteer->name ?? '-' }}</td>
                    ...
                    @empty
                    <tr><td colspan="5" style="text-align:center;padding:30px;color:var(--muted);">Belum ada data delivery</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($recentDeliveries instanceof \Illuminate\Pagination\LengthAwarePaginator && $recentDeliveries->hasPages())
            <div class="pagination-wrap">
                @if($recentDeliveries->onFirstPage())
                ...
                @else
                    <a href="{{ $recentDeliveries->previousPageUrl() }}" class="page-btn arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><polyline points="15 18 9 12 15 6"/></svg></a>
                @endif
                @foreach($recentDeliveries->getUrlRange(1, $recentDeliveries->lastPage()) as $page => $url)
                    @if($page == $recentDeliveries->currentPage())
                        <span class="page-btn active">{{ $page }}</span>
                    @elseif($page == 1 || $page == $recentDeliveries->lastPage() || abs($page - $recentDeliveries->currentPage()) <= 1)
                        <a href="{{ $url }}" class="page-btn">{{ $page }}</a>
                    @elseif(abs($page - $recentDeliveries->currentPage()) == 2)
                        <span class="page-dots">...</span>
                    @endif
                @endforeach
                @if($recentDeliveries->hasMorePages())
                    <a href="{{ $recentDeliveries->nextPageUrl() }}" class="page-btn arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><polyline points="9 18 15 12 9 6"/></svg></a>
                @else
