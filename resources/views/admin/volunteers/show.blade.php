...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .vehicle-badge { display:inline-flex; align-items:center; gap:6px; background:#dbeafe; color:#1d4ed8; font-size:0.8rem; font-weight:700; padding:5px 12px; border-radius:8px; margin-bottom:10px; }
        .vehicle-badge svg { width:14px; height:14px; }
        .vehicle-plat { font-size:0.78rem; color:var(--muted); margin-bottom:10px; }
        ...
        /* Deliveries table */
        .deliveries-section { padding:20px 22px; }
        .deliveries-section h3 { font-size:0.95rem; font-weight:700; margin-bottom:14px; }
        ...
        .btn-back-f { display:inline-flex; align-items:center; gap:6px; background:#fff; border:1px solid var(--border); color:var(--text); font-family:inherit; font-size:0.84rem; font-weight:600; padding:9px 16px; border-radius:8px; cursor:pointer; text-decoration:none; }
        ...
        .btn-back-f:hover { background:#f4f6f8; }
        .btn-del:hover { background:#fee2e2; }
        .btn-back-f svg, .btn-del svg { width:14px; height:14px; }
            ...
            <div class="top-section">
                ...
                <div class="info-col">
                    ...
                    <div class="vehicle-badge">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8.56 2.75c4.37 6.03 6.02 9.42 8.03 17.72"/></svg>
                        {{ $volunteer->vehicle_type }}
                    </div>
                    @if($volunteer->vehicle_plate)
                    <div class="vehicle-plat">Plat: {{ $volunteer->vehicle_plate }}</div>
                    @endif
                    @endif
                    <div class="info-grid">
                        <div class="info-cell">
                            <div class="info-cell-label">Total Food Delivery</div>
                            <div class="info-cell-value">{{ $volunteer->deliveries_count ?? $volunteer->deliveries->count() }}</div>
                        </div>
                        <div class="info-cell">
                            <div class="info-cell-label">Last Delivery</div>
                            <div class="info-cell-value">{{ $volunteer->deliveries->last()?->created_at?->format('d M Y') ?? '-' }}</div>
                        </div>
                        <div class="info-cell">
                        ...
                        <div class="info-cell">
                            ...
                            <div class="info-cell-value">{{ $volunteer->phone ?? '-' }}</div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- DELIVERIES HANDLED TABLE -->
            <div class="deliveries-section">
                <h3>Deliveries Handled</h3>
                <table>
                    <thead>
                        <tr>
                            <th>Food Name</th>
                    ...
                    <tbody>
                        @forelse($volunteer->deliveries as $delivery)
                        @php
                        ...
                        <tr>
                            ...
                            <td class="td-truncate">{{ $delivery->donor->name ?? '-' }}</td>
                            <td class="td-truncate">{{ $delivery->receiver->name ?? '-' }}</td>
                        ...
                        @empty
                        <tr><td colspan="6" style="text-align:center;padding:24px;color:var(--muted);">Belum ada delivery</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
