...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .stat-card { border-radius:12px; padding:16px 18px; color:#fff; position:relative; overflow:hidden; }
        ...
        .stat-sub { font-size:0.75rem; font-weight:600; opacity:.85; margin-top:3px; }
        /* Top Donor card special */
        .stat-card.purple .stat-donor-name { font-size:0.95rem; font-weight:800; margin-top:4px; }
        .stat-card.purple .stat-donor-sub  { font-size:0.72rem; opacity:.85; }
    ...
    <div class="content">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card purple">
                ...
                <div class="stat-donor-sub">{{ $stats['top_donor']->foods_count }} donations</div>
                @else
        ...
        <div class="table-card">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $donor->foods_count ?? 0 }}</td>
                        <td class="td-muted">
                            {{ $donor->foods->last()?->created_at?->format('d M Y') ?? '-' }}
                        </td>
                        <td>
                            @if(($donor->foods_count ?? 0) > 0)
                        ...
                        <td>
                            <a href="{{ route('admin.donors.show', $donor) }}" class="btn-view">
                    ...
                    @empty
                    <tr><td colspan="5" style="text-align:center;padding:30px;color:var(--muted);">Belum ada data donor</td></tr>
                    @endforelse
                </tbody>
            </table>
            @if($donors->hasPages())
            <div class="pagination-wrap">
                ...
                @else
                    <a href="{{ $donors->previousPageUrl() }}" class="page-btn arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><polyline points="15 18 9 12 15 6"/></svg></a>
                @endif
                @foreach($donors->getUrlRange(1, $donors->lastPage()) as $page => $url)
                    @if($page == $donors->currentPage())
                        <span class="page-btn active">{{ $page }}</span>
                    @elseif($page == 1 || $page == $donors->lastPage() || abs($page - $donors->currentPage()) <= 1)
                        <a href="{{ $url }}" class="page-btn">{{ $page }}</a>
                    @elseif(abs($page - $donors->currentPage()) == 2)
                        <span class="page-dots">...</span>
                    @endif
                @endforeach
                @if($donors->hasMorePages())
                    <a href="{{ $donors->nextPageUrl() }}" class="page-btn arrow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" width="14" height="14"><polyline points="9 18 15 12 9 6"/></svg></a>
                @else
